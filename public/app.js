      ...
      .then(response => response.json()).then(data=>{
        let items = data.data.items
          ...
          mainBtn.addEventListener('click',(e)=>{
            ...
            for (item in items){
              if (cityFinal == 'Todas'){
                if (items[item].prop == propFinal && items[item].type == typeFinal){
                  filterResult.push(items[item])
                }
              }else{
                if (items[item].city == cityFinal && items[item].type == typeFinal && item[items].prop == propFinal){
                  filterResult.push(items[item])
                }
              }
            ...
            setTimeout(()=>{
              ...
              if(filterResult.length != 0){
                filterResult.forEach(result => {
                  ...
                  let container = document.querySelector('.main--container')
                  container.innerHTML +=
                  `<article class="card" data-item=${result.id}>
                  <div class="card--top">
                    <div id="carousel${result.id}" data-item=${result.id} class="carousel slide" data-bs-ride="carousel" data-bs-interval="false" data-pause="hover">
                  ...
                  <div class="card--bottom">
                    ...
                    ${result.exp != 0 ? `<span class="card--bottom__exp">+${result.exp}</span>` : `<span class="card--bottom__exp">Sin Expensas</span>`}
            ...
            /*LOAD GENERAL*/
          items.forEach(item => {
            const {id,title,price,exp,mPlain, mOcc, dorm, garage,old, baths,city,type, services} = item
            container.innerHTML +=
          ...
          for (card of cards) {
            card.addEventListener("click", (e) => {
              ...
              if (child.className == 'card--bottom' || child.className=='carousel-inner'){
                ...
                const {id,title,price,exp,mPlain, mOcc, dorm, garage,old, baths,city,type, services} = modalResult
              ...
              setTimeout(()=>{
                ...
                if(filterResult.length != 0){
                  filterResult.forEach(result => {
                    /*CARGA EN PANTALLA DE RESULTADOS DEL FILTRO*/
                    let container = document.querySelector('.main--container')
                    const {id,title,price,exp,mPlain, mOcc, dorm, garage,old, baths,city,type, services} = result
                    container.innerHTML +=
                    `<article class="card" data-item=${id}>
                      <div class="card--top">
                        <div id="carousel${id}" data-item=${id} class="carousel slide" data-bs-ride="carousel" data-bs-interval="false" data-pause="hover">
